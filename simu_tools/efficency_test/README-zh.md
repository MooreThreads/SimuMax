...
FlashAttention efficiency 还有一个额外依赖边界：普通 attention shape 可以直接走 TransformerEngine；
但 `qk_head_dim` 和 `v_head_dim` 不一致的 MLA shape 需要 Megatron 的
`TEDotProductAttention` wrapper。B200 sweep 如果包含 DeepSeek/MLA shape，需要把兼容的
Megatron-LM checkout 放在 `simu_tools/megatron_scripts/Megatron-LM`，或通过
`--megatron-root` 显式指定。
...
- 如果 FA sweep 覆盖 MLA shape，确认已经准备兼容的 Megatron-LM checkout
