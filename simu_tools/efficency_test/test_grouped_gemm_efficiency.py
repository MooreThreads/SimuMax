...
def run_grouped_gemm_fwd_bwd(model, ng, M, N, K, device, dtype, MAX_TFLOPS, grad_reduce_in_bf16, save_root):
    # single_out_shape = ( # (M, N) x ng-> (ng * M, N) concat all experts' output
    json_path = os.path.join(save_root, 'grouped_gemm_efficiency.json')
    ...
    if any([all_efficiency[matmul_key]['accurate_efficient_factor'].get(fwd_shape_str, None) is None, 
        ...
        print(f'=== [{model} Groupedgemm] skip all!!!!!!!!!!!!')
        return
    # fwd
    ...
    if all_efficiency[matmul_key]['accurate_efficient_factor'].get(fwd_shape_str, None) is None:
        ...
        fwd_efficiency = fwd_TFLOPS / MAX_TFLOPS
        
        all_efficiency[matmul_key]['accurate_efficient_factor'][fwd_shape_str] = fwd_efficiency
        print(f"=== [{model} Groupedgemm] -- [fwd] Running grouped gemm with ng={ng}, M={M}, N={N}, K={K}, dtype={dtype}, out_dtype={fwd_out_dtype}, main_grad_dtype={main_grad_dtype}, device={device}, TFLOPS={fwd_TFLOPS}, dur={fwd_dur} ms, fwd_efficiency={fwd_efficiency}")
    else:
        print(f'=== [{model} Groupedgemm] -- [fwd] skip {fwd_shape_str}')
    # bwd_grad_act
    ...
    if all_efficiency[matmul_key]['accurate_efficient_factor'].get(bwd_grad_act_shape_str, None) is None:
        ...
        bwd_grad_act_efficiency = bwd_grad_act_TFLOPS / MAX_TFLOPS
        all_efficiency[matmul_key]['accurate_efficient_factor'][bwd_grad_act_shape_str] = bwd_grad_act_efficiency
        print(f"=== [{model} Groupedgemm] -- [bwd_grad_act] Running grouped gemm with ng={ng}, M={M}, N={N}, K={K}, dtype={dtype}, out_dtype={fwd_out_dtype}, main_grad_dtype={main_grad_dtype}, device={device}, TFLOPS={bwd_grad_act_TFLOPS}, dur={bwd_grad_act_dur} ms, bwd_grad_act_efficiency={bwd_grad_act_efficiency}")
    else:
        print(f'=== [{model} Groupedgemm] -- [bwd_grad_act] skip {bwd_grad_act_shape_str}')
    ...
    if all_efficiency[matmul_key]['accurate_efficient_factor'].get(bwd_grad_weight_shape_str, None) is None:
        ...
        bwd_grad_w_efficiency = bwd_grad_w_TFLOPS / MAX_TFLOPS
        all_efficiency[matmul_key]['accurate_efficient_factor'][bwd_grad_weight_shape_str] = bwd_grad_w_efficiency
        print(f"=== [{model} Groupedgemm] -- [bwd_grad_w] Running grouped gemm with ng={ng}, M={M}, N={N}, K={K}, dtype={dtype}, out_dtype={fwd_out_dtype}, main_grad_dtype={main_grad_dtype}, device={device}, TFLOPS={bwd_grad_w_TFLOPS}, dur={bwd_grad_w_dur} ms, bwd_grad_w_efficiency={bwd_grad_w_efficiency}")
    else:
        print(f"=== [{model} Groupedgemm] -- [bwd_grad_w] Skip  skip {bwd_grad_weight_shape_str}")
    ...
    json.dump(all_efficiency, open(f'{save_root}/grouped_gemm_efficiency.json', 'w'), indent=4)
...
def test_grouped_gemm_efficiency(batch, seq_len, num_experts, ep_size, topk, capacity, hidden_size, moe_intermediate_size, device, MAX_TFLOPS, dtype, grad_reduce_in_bf16, save_root, model):
    ...
    print(f"=== [{model} Groupedgemm] Run test for batch={batch}, seq_len={seq_len}, ep_size={ep_size}, num_experts={num_experts}, topk={topk}, capacity={capacity}, hidden_size={hidden_size}, moe_intermediate_size={moe_intermediate_size}, Linear1 device={device}, dtype={dtype}, grad_reduce_in_bf16={grad_reduce_in_bf16}")
    ...
    print(f"=== [{model} Groupedgemm] Run test for batch={batch}, seq_len={seq_len}, ep_size={ep_size}, num_experts={num_experts}, topk={topk}, capacity={capacity}, hidden_size={hidden_size}, moe_intermediate_size={moe_intermediate_size}, Linear2 device={device}, dtype={dtype}, grad_reduce_in_bf16={grad_reduce_in_bf16}")
    run_grouped_gemm_fwd_bwd(
        model = model,
        ng = num_local_expert,
        M = num_tokens_per_local_expert,
...
def test(dtype, grad_reduce_in_bf16):
    ...
    save_root = get_efficiency_save_root(system, 'grouped_gemm_efficiency')
                ...
                for ep_size in EP_LIST:
                        for model_config in MODEL_CONFIGS:   
                            if model_config.expert_num > 1:
                                if model_config.expert_num % ep_size != 0:
                                    continue
